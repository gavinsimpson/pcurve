"pcget.lam"<- function(x, s, latent = FALSE, stretch = 2, uni.lam,
                       tag) {
    ...
    ##
    tt <- .C("getlam",                  # C argument names, where different / not named
             as.integer(n),             # nn
             as.integer(p),             # pp
             xt,                        # x
             s = xt,                    # sx
             ...
             tag = integer(n),
             dist = double(n),
             as.integer(nrow(s)),       # nns
             st,                        # s
             as.double(stretch),
             as.double(uni.lam),
             vecx = double(p),
             tempsx = double(p),
             PACKAGE = "pcurve")[c("s", "tag", "lambda", "dist")]
    if (latent) {
